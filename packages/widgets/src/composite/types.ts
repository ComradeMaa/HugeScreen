...
/** CSS grid-template-areas for each template (4×4 internal grid) */
...
const TEMPLATE_COLUMNS: Record<CompositeLayoutTemplate, number> = {
  '2col': 4, '2row': 4, '3col': 6, '2x2': 4, '1top2bottom': 4, '1left2right': 4,
};
...
export function isTemplateViableForSize(
  ...
  const minCols: Record<CompositeLayoutTemplate, number> = {
    '2col': 3, '2row': 2, '3col': 4, '2x2': 4, '1top2bottom': 3, '1left2right': 3,
  };
  const minRows: Record<CompositeLayoutTemplate, number> = {
    '2col': 2, '2row': 3, '3col': 2, '2x2': 3, '1top2bottom': 3, '1left2right': 3,
  };
